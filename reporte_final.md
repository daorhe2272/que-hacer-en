...
El núcleo de procesamiento utiliza la API de **Google Gemini** (`gemini-3.1-flash-lite`). Una característica clave empleada es la **Salida Estructurada** (*Structured Output*), que fuerza al modelo a responder estrictamente en formato **JSON** (un formato de texto estándar para organizar e intercambiar datos), garantizando que la información extraída sea computable y consistente.
